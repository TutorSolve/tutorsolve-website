...
def setup_indexes(db):
    ...
    db.payments.create_index(
        ...
        name="payments_refund_queue_idx",
        partialFilterExpression={"status": "refund_requested"}
    )
    db.payments.create_index(
        [("status", 1), ("refunded_at", -1)],
        name="payments_refunded_history_idx",
        partialFilterExpression={"status": "refunded"}
    )
    print("payments: index")

    db.payouts.create_index("expert_id")
    db.payouts.create_index("is_paid")
    db.payouts.create_index([("question_id", 1), ("is_paid", 1)], name="payouts_question_is_paid_idx")
