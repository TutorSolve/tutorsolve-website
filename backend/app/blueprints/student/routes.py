...
def request_refund(question_id):
    ...
    if payment.get("status") == "refunded":
        return jsonify({"error": "This order has already been refunded"}), 409

    now = datetime.utcnow()

    # Determine payment stage for context-aware messaging
    is_post_completion = payment.get("completion_paid", False)

    if is_post_completion:
        auto_msg_body = (
            "⚠️ [REFUND REQUEST — POST COMPLETION] The student has requested a refund "
            "after the completion payment for this order. Please review the situation "
            "and initiate a refund if valid."
        )
        notif_body = f"A student has requested a post-completion refund for: {question['title']}"
    else:
        auto_msg_body = (
        ...
        notif_body = f"A student has requested a refund for: {question['title']}"

    # DO NOT mark status as refund_requested here.
