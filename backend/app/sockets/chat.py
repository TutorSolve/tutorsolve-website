...
def _can_join(identity, thread):
    ...
    Thread E — Expert  ↔ Super Admin direct chat.
    Thread F — Employee ↔ Super Admin direct chat.
...
def _mark_thread_read(db, thread, identity, now=None):
    ...
    if updates:
        db.threads.update_one({"_id": thread["_id"]}, {"$set": updates})
...
def on_join(data):
    ...
    _mark_thread_read(db, thread, identity)
