(function () {
  ...
  const badgeEl = document.getElementById("sa-admin-chat-unread-badge");
  if (!badgeEl) return;

  const token = localStorage.getItem("ts_token");
  if (!token) return;

  const onChatPage = window.location.pathname.includes("/admin/super-admin-chat");
  ...
  async function refreshUnreadCount() {
    ...
    isRefreshing = true;
    try {
      const res = await apiFetch("/admin/super-admin-chat/unread-count");
  ...
  function scheduleRefresh(delayMs) {
    ...
    pollTimer = setTimeout(refreshUnreadCount, delayMs || 120);
  }
  ...
  function onSocketNotification(data) {
    ...
    scheduleRefresh(onChatPage ? 250 : 100);
  }
